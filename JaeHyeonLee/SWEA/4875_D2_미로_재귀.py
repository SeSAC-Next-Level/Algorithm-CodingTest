...
def DFS(r, c):
    ...
    if 인접행렬[r][c] == 3:
        ans = 1
        return 
    인접행렬[r][c] = 1 # 방문지를 대신 함
    
    for d in range(4):
        nr, nc = r + dr[d], c + dc[d]
        if 0 <= nr < 정점 and 0 <= nc < 정점 and 인접행렬[nr][nc] != 1:
            DFS(nr, nc)
        
for ts in range(1, T + 1):
    정점 = int(input())
    인접행렬 = [list(map(int, input().rstrip())) for _ in range(정점)]


    ans = 0
    # 예정지, 방문지
    # 예정지 = deque() # 재귀로 하면 시스탬 스택에 쌓임
    # 방문지 = set()
    ...
    #         ans = 1
    #         break
    #     인접행렬[r][c] = 1 # 방문지를 대신할 수 있음
    #     for n in range(4):
    #         nr, nc =  r + dr[n], c + dc[n]
    #         # if 0 <= nr < 정점 and 0 <= nc < 정점 and 인접행렬[nr][nc] != 1 and (nr, nc) not in 방문지 :
    #         if 0 <= nr < 정점 and 0 <= nc < 정점 and 인접행렬[nr][nc] != 1 :
    #             예정지.append((nr, nc))
    #             # 방문지.add((nr, nc))
